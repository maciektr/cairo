...
fn handle_mod(db: &dyn SyntaxGroup, module_ast: ast::ItemModule) -> PluginResult {
    ...
    let mut event_wrappers = Vec::new();
    ...
    for item in body.items(db).elements(db) {
        let keep_original = match &item {
                    ...
                    || item_function.has_attr(db, VIEW_ATTR) =>
            {
                ...
                // TODO(yg): another PR - change the order - don't add to ABI if wrapper generation
                // failed.
            ...
            ast::Item::FreeFunction(item_function) if item_function.has_attr(db, EVENT_ATTR) => {
                ...
                if let Some((event_wrapper_rewrite, abi_event_rewrite)) = rewrite_nodes {
                    event_wrappers.push(event_wrapper_rewrite);
    ...
    let generated_contract_mod = RewriteNode::interpolate_patched(
        ...
        HashMap::from([
            ...
            (
                "events".to_string(),
                RewriteNode::Modified(ModifiedNode { children: event_wrappers }),
            ),
        ]),
    );
...
/// Generates an event function.
/// Returns a RewriteNode for the event wrapper, a RewriteNode for the ABI declaration and
/// diagnostics.
fn handle_event(
    db: &dyn SyntaxGroup,
    ...
    if matches!(ret_ty, OptionReturnTypeClause::ReturnTypeClause(_)) {
        diagnostics.push(PluginDiagnostic {
            ...
            message: "Event functions must return nothing.".to_string(),
        });
    }
    ...
    for param in signature.parameters(db).elements(db) {
        ...
        let Some((ser_func, _)) = get_type_serde_funcs(&type_name) else {
            diagnostics.push(PluginDiagnostic {
    ...
    if function_ast.body(db).statements(db).elements(db).len() > 0 {
        diagnostics.push(PluginDiagnostic {
